...
extension PlaygroundBook {
    func build(command: String, in webSocket: WebSocketOutput) -> IO<PlaygroundBookCommandError, PlaygroundBookGenerated> {
        let config = configuration(webSocket: webSocket)
        
        return build(command: command)
            .provide(config)
            .report(withConfig: config.webSocketConfig)
    }
